...
// Parse de string "YYYY-MM-DD" no fuso LOCAL
// (evita o off-by-one do toISOString).
...
// NUNCA usar toISOString aqui: em fusos negativos como
// America/Sao_Paulo isso pode devolver o dia anterior.
const ymd = (d: Date) => {
...
const diffDays = (a: Date, b: Date) =>
  Math.round(
    (startOfDay(b).getTime() - startOfDay(a).getTime()) / DAY
  );
...
const isoWeek = (d: Date) => {
  ...
  const yearStart = new Date(
    Date.UTC(x.getUTCFullYear(), 0, 1)
  );

  const week = Math.ceil(
    (((x.getTime() - yearStart.getTime()) / DAY) + 1) / 7
  );
...
function FaturamentoPage() {
  ...
  // Soma por dia (mapa data -> faturamento).
  // Chave = string crua "YYYY-MM-DD" exatamente como
  // vem do banco — sem conversão de fuso.
  const dailyMap = useMemo(() => {
    ...
    for (const v of vendas) {
      ...
      // garante que pegamos apenas "YYYY-MM-DD"
      // mesmo se vier "YYYY-MM-DDT..."
  ...
  // Para um comparativo justo, o período anterior usa o MESMO
  // número de dias já decorridos do período atual.
  const now = new Date();
  const today = startOfDay(now);
  ...
  const daysIntoWeek =
    diffDays(weekStart, today);

  const lastWeekStart =
    addDays(weekStart, -7);

  const lastWeekSameOffset =
    addDays(
      lastWeekStart,
      daysIntoWeek
    );
  ...
  const lastMonthEndFull =
    addDays(monthStart, -1);

  // hoje.day pode não existir em meses curtos
  // (ex.: 31 jan vs 28 fev) — clamp.
  const lastMonthSameOffset =
    new Date(
      Math.min(
        new Date(
  ...
  // mesmo mês/dia do ano anterior
  // (clamp para 28/02 em ano não-bissexto).
  ...
  const monthly = useMemo(() => {
    ...
    // mensal usa TODO o histórico para
  ...
  return (
    <AppShell
      ...
      subtitle="Visão gerencial e comparativos de crescimento"
    >
      {empty ? (
        <EmptyState
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <DeltaKpi
              ...
              value={kpis.hoje.v}
              previous={kpis.hoje.p}
              hint="vs ontem"
            />

            <DeltaKpi
              label="Faturamento Esta Semana"
              value={kpis.semana.v}
              previous={kpis.semana.p}
          ...
          >
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-sm text-muted-foreground">
                ...
                >
                  {(tendencia.ratio * 100).toLocaleString(
                    "pt-BR",
                    {
                      maximumFractionDigits: 1,
                    }
              ...
              <span className="text-sm text-muted-foreground">
                1ª metade:{" "}
                {brl(tendencia.firstHalf)}
                {" · "}
                2ª metade:{" "}
                {brl(tendencia.secondHalf)}
              </span>
            </div>
          </Section>
          ...
          >
            ...
            {chartTab === "mensal" && (
              <p className="mt-2 text-xs text-muted-foreground">
                Histórico completo — independente do filtro acima.
              </p>
            )}

            {chartTab === "anual" && (
              <p className="mt-2 text-xs text-muted-foreground">
                Comparativo entre todos os anos com vendas.
              </p>
            )}
          </Section>
        </>
