...
function InsightsPage() {
  ...
  const insights: Insight[] = useMemo(() => {
    ...
    const menosLucro = [...arr].sort(
      (a, b) => a.lucro - b.lucro
    )[0];

    const maiorGiro = [...arr].sort(
      (a, b) => b.qtd - a.qtd
    )[0];
    ...
    if (maisLucro) {
      out.push({
        ...
        body: `“${maisLucro.produto}” gerou ${brl(
          maisLucro.lucro
        )} de lucro em ${num(
          maisLucro.qtd
        )} unidades.`,
    ...
    if (
      ...
      out.push({
        ...
        body: `“${menosLucro.produto}” acumula prejuízo de ${brl(
          menosLucro.lucro
        )}. Reveja preço de venda ou custo.`,
      });
    }
    ...
    if (maiorGiro) {
      out.push({
        ...
        body: `“${maiorGiro.produto}” teve ${num(
          maiorGiro.qtd
        )} unidades vendidas.`,
      });
    }
    ...
    if (parado) {
      out.push({
        ...
        body: `“${parado.descricao}” está com ${num(
          Number(parado.estoque_atual)
        )} unidades sem vendas registradas.`,
      });
    }
      ...
      out.push({
        ...
        title:
          delta >= 0
            ...
            : "Tendência de queda",
        body: `Faturamento do último mês variou ${pct(
          delta
        )} em relação ao anterior (${brl(
          cur
        )} vs ${brl(prev)}).`,
      });
    }
    ...
    if (saldo < 0) {
      out.push({
        ...
        body: `Seu fluxo de caixa está em ${brl(
          saldo
        )}. Avalie reduzir saídas ou aumentar entradas.`,
      });
    }
  ...
  return (
    <AppShell
      ...
      subtitle="Resumo automático para o gestor"
    >
      <Section
        title="Destaques do negócio"
        actions={
      ...
      >
        {insights.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Importe dados para gerar insights automáticos.
          </p>
        ) : (
